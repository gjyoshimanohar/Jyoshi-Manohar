  ...
  return (
    <main className="min-h-screen pt-28 pb-20 bg-slate-50/50">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-50">
          ...
          <div className="flex flex-col items-end gap-3">
            ...
            {isAdmin && (
              <div className={`flex items-center gap-2 bg-primary text-white px-3 py-1.5 rounded-xl text-sm font-sans tracking-normal shadow-md animate-in fade-in slide-in-from-right-2 duration-300 border border-primary/20 ${!selectedClientId ? 'opacity-80' : ''}`}>
                 <span className="relative flex h-2 w-2 shrink-0">
                   {selectedClientId ? <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span> : null}
                   <span className={`relative inline-flex rounded-full h-2 w-2 ${selectedClientId ? 'bg-amber-500' : 'bg-slate-400'}`}></span>
                 </span>
                 <span className="font-semibold px-1 shrink-0">Active client:</span>
                 <CustomSelect
                   value={selectedClientId || ""}
                   onChange={(val) => {
                     if (val) {
                       setSelectedClientId(val);
                       setSelectedClientEmail(clients.find(c => c.uid === val)?.email || '');
                     } else {
                       setSelectedClientId('');
                       setSelectedClientEmail('');
