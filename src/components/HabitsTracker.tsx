...
import { Target, Plus, Flame, Sparkles, Trash2, Trophy } from 'lucide-react';
  ...
  return (
    <div className="w-full flex flex-col h-full bg-white select-none">
      <div className="mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black text-gray-900 flex items-center">
            <Target className="w-5 h-5 mr-2 text-[#2F6BE6]" />
            Habit Ring Tracker
          </h2>
        ...
        {/* Global Stats Board */}
        <div className="flex space-x-3 bg-[#2F6BE6]/5 border border-[#2F6BE6]/10 p-3 rounded-xl items-center">
          <Trophy className="w-6 h-6 text-[#2F6BE6]" />
          <div>
            <span className="text-[10px] font-bold text-[#2F6BE6] uppercase block tracking-wider">Total Active Habits</span>
            <span className="text-sm font-black text-[#2F6BE6]">{habits.length} Tracking</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 bg-gray-50 border border-gray-100 p-4 rounded-xl">
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2.5">Quick Presets</h3>
        <div className="flex flex-wrap gap-2">
          {PRESET_HABITS.map(p => (
            <button
              key={p.name}
              onClick={() => handleAddPreset(p)}
              className="px-3 py-1.5 text-xs font-semibold bg-white border border-gray-200 hover:border-[#2F6BE6] hover:text-[#2F6BE6] hover:scale-102 rounded-full transition-all flex items-center space-x-1.5 shadow-sm active:scale-98"
            >
              <span>{p.icon}</span>
              <span>{p.name}</span>
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                <div className="flex items-center space-x-3.5 mb-3 sm:mb-0 w-full sm:w-auto">
                  ...
                  <div className="text-left">
                    <h4 className="font-bold text-sm text-gray-900 leading-tight">{habit.name}</h4>
                    <div className="flex items-center space-x-2 mt-1">
                      <span className="text-[10px] bg-amber-50 text-amber-700 font-bold px-1.5 py-0.5 rounded-full flex items-center">
                    ...
                    {lastDays.map(day => {
                      ...
                      return (
                        <div key={day.key} className="flex flex-col items-center">
                          <span className={`text-[9px] font-bold uppercase mb-1 ${day.isToday ? 'text-[#2F6BE6]' : 'text-gray-400'}`}>
                            {day.label}
                          </span>
                          <button
                            onClick={() => toggleCheckedDay(habit.id, day.key)}
                            className={`w-8 h-8 rounded-full border flex items-center justify-center font-bold text-xs shadow-sm transition-all duration-200 active:scale-90 ${checked ? 'bg-[#2F6BE6] text-white border-[#2F6BE6]' : 'bg-gray-50 border-gray-200 hover:bg-gray-100 text-gray-500'}`}
                          >
                            {checked ? '✓' : ''}
                          </button>
                        </div>
        ...
        <div className="bg-gray-50 border border-gray-100 p-5 rounded-xl h-fit">
          <h3 className="font-black text-xs text-gray-500 uppercase tracking-widest mb-4 flex items-center">
            <Sparkles className="w-3.5 h-3.5 mr-1.5 text-amber-500" />
            Custom Creator
          </h3>
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Habit Title</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Launch Icon/Emoji</label>
              <select
                value={newHabitIcon}
            ...
            <button
              ...
              className="w-full bg-[#2F6BE6] hover:bg-[#1a56cc] disabled:opacity-50 text-white py-2 px-4 rounded-lg text-xs font-bold transition-all shadow-md mt-2"
            >
              Add Custom Habit
            </button>
          </div>
