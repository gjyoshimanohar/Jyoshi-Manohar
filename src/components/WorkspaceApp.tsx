...
const renderIcon = (
  ...
  if (name && name.length <= 4) {
    return (
      <span
        className={`${className} flex items-center justify-center text-lg leading-none`}
      >
        {name}
      </span>
    );
...
const renderHighlightedTitleForInput = (
  title: string,
    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === repeatInterval.toLowerCase() ? (
          <span
            key={i}
            className="bg-primary/10 text-primary px-[3px] -mx-[3px] rounded"
          >
            {part}
          </span>
        ) : (
          <span key={i} className="text-[#202020]">
            {part}
          </span>
        ),
      )}
...
function parseMarkdown(text: string): string {
  ...
  html = html.replace(
    /`(.*?)`/g,
    '<code class="bg-gray-100 text-rose-600 px-1.5 py-0.5 rounded font-mono text-xs">$1</code>',
  );
...
export default function WorkspaceApp() {
  ...
  const handleFolderDrop = async (
    ...
    if (draggedFolderId && draggedFolderId !== targetFolderId) {
      ...
      if (draggedFolder && targetFolder) {
        ...
        await todoService.updateFolder(draggedFolderId, {
          order: targetOrder - 1,
        });
      }
    }
  ...
  const handleProjectDropReorder = async (
    e: React.DragEvent,
  ...
  const handleExportTasksCSV = () => {
    ...
    triggerCSVDownload(
      csvContent,
      `tasks_backup_${new Date().toISOString().split("T")[0]}.csv`,
    );
  };
  ...
  const handleExportProjectsCSV = () => {
    ...
    triggerCSVDownload(
      csvContent,
      `projects_backup_${new Date().toISOString().split("T")[0]}.csv`,
    );
  };
  ...
  const handleExportFoldersCSV = () => {
    ...
    triggerCSVDownload(
      csvContent,
      `folders_backup_${new Date().toISOString().split("T")[0]}.csv`,
    );
  };
  ...
  const [isProgressBannerExpanded, setIsProgressBannerExpanded] =
    useState(false);
  ...
  const [editingProjectIdInModal, setEditingProjectIdInModal] = useState<
    string | null
  >(null);
  ...
  const [deletingTodoState, setDeletingTodoState] = useState<{
    id: string;
    viewModeAtTime: ViewMode;
  } | null>(null);
  const [taskDeclineOrDeleteReason, setTaskDeclineOrDeleteReason] =
    useState<string>("");
  ...
  const [editProjectFolderId, setEditProjectFolderId] = useState<string | null>(
    null,
  );
  const [showEditEmojiPicker, setShowEditEmojiPicker] = useState(false);

  // Detail Drawer pickers
  const [editingTodoDateId, setEditingTodoDateId] = useState<string | null>(
    null,
  );
  const [editingTodoDeadlineId, setEditingTodoDeadlineId] = useState<
    string | null
  >(null);
  ...
  const handleInlineEditSubmit = () => {
    if (inlineEditTaskId && inlineEditTitle.trim()) {
      ...
      if (todo && todo.title !== inlineEditTitle.trim()) {
        todoService.updateTodo(inlineEditTaskId, {
  ...
  const [showEditFolderColorPicker, setShowEditFolderColorPicker] =
    useState(false);
  ...
  const [newTaskRepeatInterval, setNewTaskRepeatInterval] = useState<
  ...
  const [activeAddingSection, setActiveAddingSection] = useState<string | null>(
    null,
  );
  ...
  const ensureTickTickBootstrap = async (uid: string) => {
    ...
    try {
      ...
      if (studyFolder) {
        ...
        const research = await todoService.createProject(
          "Research Items",
          "#3b82f6",
          uid,
          "Book",
        ...
        if (research) {
          await todoService.updateProject(research.id, {
        ...
        const caFinal = await todoService.createProject(
          "CA Final",
          "#22c55e",
          uid,
          "Award",
        ...
        if (caFinal) {
          await todoService.updateProject(caFinal.id, {
            folderId: studyFolder.id,
          });
        ...
        const spiritual = await todoService.createProject(
          "Spiritual",
          "#d946ef",
          uid,
          "Heart",
        ...
        if (spiritual) {
          await todoService.updateProject(spiritual.id, {
            folderId: studyFolder.id,
          });
          await todoService.createTodo({
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsubProjects = todoService.subscribeToProjects(
        auth.currentUser.uid,
        (fetchedProjects) => {
          setProjects(fetchedProjects);
        },
      );
      unsubFolders = todoService.subscribeToFolders(
        auth.currentUser.uid,
        (fetchedFolders) => {
  ...
  const getCompletionDate = (t: Todo) => {
    ...
    const act = t.activities
      ?.slice()
      .reverse()
      .find((a) => a.type === "status" && a.newValue === "Completed");
    ...
    const checkReminders = () => {
      ...
      todos.forEach((todo) => {
        if (
          !todo.completed &&
          todo.dueDate &&
          !notifiedTaskIds.current.has(todo.id)
        ) {
  ...
  const handleClearAllData = async () => {
    if (
      confirm(
        "Are you sure you want to clear ALL your tasks, projects, and folders? This cannot be undone.",
      )
    ) {
      try {
        ...
        localStorage.removeItem(
          `ticktick_bootstrapped_${auth.currentUser?.uid}`,
        );
  ...
  const handleAddTask = async () => {
    ...
    const { projectId: targetProjectId, matchedProjectName } =
      determineProjectByTitle(newTaskTitle.trim(), projects, currentBaseProjId);
    ...
    await todoService.createTodo({
      ...
      clientName: newTaskClientId
        ? clients.find((c) => c.uid === newTaskClientId)?.displayName ||
          clients.find((c) => c.uid === newTaskClientId)?.email
        : undefined,
    });
  ...
  const handleAddTodayTask = async (title: string) => {
    ...
    const currentBaseProjId =
      viewMode === "project" && selectedProjectId ? selectedProjectId : "inbox";
    const { projectId: targetProjectId, matchedProjectName } =
      determineProjectByTitle(title.trim(), projects, currentBaseProjId);
  ...
  const handleMoveProjectToFolder = async (
    projectId: string,
    folderId: string | null,
  ) => {
    await todoService.updateProject(projectId, { folderId });
  ...
  const handleDeleteFolder = async (folderId: string, e: React.MouseEvent) => {
    ...
    setConfirmDialog({
      isOpen: true,
      message:
        "Are you sure you want to delete this folder? Inside projects will be disconnected.",
      onConfirm: async () => {
  ...
  const handleToggleTodo = async (todo: Todo) => {
    if (!todo.completed && getActiveDependenciesCount(todo) > 0) {
      toast(
        "Cannot complete this task. It is blocked by " +
          getActiveDependenciesCount(todo) +
          " active task(s).",
      );
      return;
    }
    ...
    if (isCompleting && todo.repeatInterval) {
      ...
      const isAlreadyScheduled = todos.some(
        (t) =>
          !t.completed &&
          !t.deletedAt &&
          ...
          t.dueDate &&
          startOfDay(new Date(t.dueDate)).getTime() === nextDueDateTimestamp,
      );
  ...
  const handleSaveRenameSection = async (oldName: string) => {
    ...
    const updatedSections = (currentProj.sections || []).map((s) =>
      s === oldName ? newName : s,
    );
    await todoService.updateProject(selectedProjectId, {
      sections: updatedSections,
    ...
    const matchingTodos = todos.filter(
      (t) => t.projectId === selectedProjectId && t.sectionName === oldName,
    );
  ...
  const handleDeleteSection = async (sectionName: string) => {
    ...
    const updatedSections = (currentProj.sections || []).filter(
      (s) => s !== sectionName,
    );
    await todoService.updateProject(selectedProjectId, {
      sections: updatedSections,
    ...
    const matchingTodos = todos.filter(
      (t) => t.projectId === selectedProjectId && t.sectionName === sectionName,
    );
    for (const t of matchingTodos) {
  ...
  const handleAddTaskToSection = async (
    sectionName: string,
    titleStr: string,
    tagsStr: string = "",
    descStr: string = "",
  ...
  const handleConfirmDeleteTodo = async () => {
    ...
    try {
      const reasonToSave =
        taskDeclineOrDeleteReason.trim() || "No longer applicable or required";
    ...
    switch (viewMode) {
      ...
      case "today":
        return baseTodos.filter((t) => {
          ...
          return (
            (due && (isToday(due) || isPast(due))) ||
            (dl && (isToday(dl) || isPast(dl)))
          );
        });
      case "upcoming":
        return baseTodos.filter((t) => {
          ...
          const dl = t.deadline ? new Date(t.deadline) : null;
          return (
            (due && (!isPast(due) || isToday(due))) ||
            (dl && (!isPast(dl) || isToday(dl)))
          );
        });
      case "project":
        return baseTodos.filter((t) => t.projectId === selectedProjectId);
      case "folder":
        const folderProjectIds = projects
          .filter((p) => p.folderId === selectedFolderId)
          .map((p) => p.id);
        return baseTodos.filter(
          (t) => t.projectId && folderProjectIds.includes(t.projectId),
        );
      case "completed":
      case "trash":
      case "trends":
  ...
  const getPriorityColor = (priority?: number) => {
    switch (priority) {
      case 1:
        return "text-red-400 fill-red-400/40 hover:text-red-500 hover:fill-red-400/60";
      case 2:
        return "text-orange-400 fill-orange-400/40 hover:text-orange-500 hover:fill-orange-400/60";
      case 3:
        return "text-blue-400 fill-blue-400/40 hover:text-blue-500 hover:fill-blue-400/60";
      default:
  ...
  const getPriorityCheckboxStyle = (priority?: number, isOverdue = false) => {
    if (isOverdue)
      return "border-[#de4c4a] hover:bg-red-50/50 text-[#de4c4a] bg-transparent";
    switch (priority) {
      case 1:
        return "border-red-400 hover:bg-red-50/50 text-red-500 bg-transparent";
      case 2:
        return "border-orange-400 hover:bg-orange-50/50 text-orange-500 bg-transparent";
      case 3:
        return "border-blue-400 hover:bg-blue-50/50 text-blue-500 bg-transparent";
      default:
        return "border-gray-300 hover:bg-gray-50 text-gray-400 bg-transparent";
    }
  };
  ...
  const getProjectPendingCount = (projectId: string) => {
    return todos.filter(
      (t) => !t.completed && !t.deletedAt && t.projectId === projectId,
    ).length;
  };
  ...
  const renderItemProjectBadge = (todo: Todo) => {
    ...
    return (
      <span className="flex items-center text-xs sm:text-xs font-semibold text-gray-500 bg-transparent transition-colors leading-none shrink-0 border border-gray-100 px-1.5 py-0.5 rounded-full select-none">
        ...
        />
        <span style={{ color: p.color ? `${p.color}e0` : "#6B7280" }}>
          {p.name}
        </span>
      </span>
    );
        ...
        <div className="flex items-center min-w-0 flex-1">
          ...
          >
            <span className="text-xs sm:text-sm text-[#202020] font-normal leading-relaxed flex items-center gap-1.5">
              {todo.repeatInterval && (
                <RefreshCw className="inline-block w-3 h-3 text-primary flex-shrink-0" />
              )}
              {getActiveDependenciesCount(todo) > 0 ? (
                <span
                  className="flex items-center gap-0.5 text-rose-600 bg-rose-50 px-1 py-[1px] rounded shrink-0"
                  title={`Waiting on ${getActiveDependenciesCount(todo)} task(s)`}
                >
              ...
              {inlineEditTaskId === todo.id ? (
                <input
                  ...
                  className={
                    "bg-white border border-blue-300 rounded px-1 text-inherit font-normal outline-none focus:ring-2 focus:ring-blue-100 flex-1 min-w-0 " +
            ...
            {todo.description && (
              <p className="text-xs text-gray-500 line-clamp-1 leading-normal font-medium mt-0.5">
                {todo.description}
              </p>
            )}
            {todo.subtasks && todo.subtasks.length > 0 && (
              <div
                className="mt-1.5 flex items-center gap-1.5 text-xs text-gray-400 font-medium select-none"
                onClick={(e) => e.stopPropagation()}
              >
  ...
  const renderProjectItem = (project: Project) => (
    <div
      ...
      className="group relative flex items-center justify-between pl-2 cursor-grab active:cursor-grabbing"
    >
      {editingProjectId === project.id ? (
        <form
          onSubmit={(e) => handleSaveEditProject(project.id, e)}
          className="flex items-center space-x-2 w-full p-1 bg-white border border-gray-100 rounded shadow-sm z-10"
        >
          <input
            type="text"
            autoFocus
          ...
          <button
            ...
            className="p-1 text-white bg-primary rounded"
          >
            ...
            onDrop={async (e) => {
              ...
              if (taskId) {
                ...
                setTodos((prev) =>
                  prev.map((t) =>
                    t.id === taskId
                      ? { ...t, projectId: project.id, sectionName: null }
                      : t,
                  ),
                );
              ...
              <div className="absolute right-0 top-full mt-1 w-36 bg-white border-none rounded-xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] p-1.5 z-50 text-sm animate-in fade-in duration-100">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setActiveProjectMenu(null);
                    setEditingProjectIdInModal(project.id);
                    setNewProjectName(project.name);
                    setListColor(project.color || "#1a2b58");
                    setListFolderId(project.folderId || "none");
                    setListViewType(project.viewType || "list");
                    setIsProjectModalOpen(true);
                  }}
                  className="w-full text-left px-2.5 py-2 hover:bg-slate-50 text-slate-700 rounded-lg font-medium transition-colors"
                >
                  Edit
                </button>
                <button
                  onClick={async (e) => {
                    e.stopPropagation();
                    setActiveProjectMenu(null);
                    await todoService.updateProject(project.id, {
                ...
                <button
                  ...
                  className="w-full text-left px-2.5 py-2 hover:bg-red-50 text-red-600 rounded-lg font-medium transition-colors"
                >
                  Delete
                </button>
              </div>
  ...
  const renderProjectList = () => (
    <div className="space-y-2 mt-1">
        ...
        >
          {editingFolderId === folder.id ? (
            <form
              ...
              className="flex items-center space-x-1 p-1 bg-white border border-gray-100 rounded-lg shadow-sm"
            >
              ...
              <button
                ...
                className="text-white bg-primary p-1 rounded-md shrink-0"
              >
              ...
              <button
                ...
                className="text-gray-400 hover:bg-gray-100 p-1 rounded-md shrink-0"
              >
            ...
            >
              ...
              >
                ...
                >
                  <ChevronDown
                    className={`w-3.5 h-3.5 text-gray-400 hover:text-gray-600 shrink-0 transition-transform ${expandedFolders.includes(folder.id) ? "" : "-rotate-90"}`}
                  />
                </div>
                <Folder className="w-3.5 h-3.5 text-primary shrink-0" />
                <span
                  className={`text-sm font-semibold truncate max-w-[120px] ${viewMode === "folder" && selectedFolderId === folder.id ? "text-gray-900 font-semibold" : "text-gray-700"}`}
                >
                  {folder.name}
                </span>
          ...
          {expandedFolders.includes(folder.id) && (
            <div className="pl-3.5 border-l border-gray-200 ml-3.5 mt-1 space-y-1.5">
              ...
              {projects.filter((p) => p.folderId === folder.id).length ===
                0 && (
                <div className="text-sm text-gray-400 pl-1 py-1 italic">
                  Empty folders list
                </div>
              )}
            </div>
      ...
      <div
        ...
        onDragOver={handleDragOver}
        className="min-h-[20px] space-y-1.5 mt-1.5"
      >
      ...
      {projects.length === 0 && folders.length === 0 && !isProjectModalOpen && (
        <div className="text-sm text-center text-gray-400 italic py-2">
          No lists created
        </div>
      )}
    </div>
  ...
  const inboxCount = todos.filter(
    (t) =>
      !t.completed && !t.deletedAt && (!t.projectId || t.projectId === "inbox"),
  ).length;
  ...
  return (
    <div className="flex flex-1 overflow-hidden h-full relative font-sans text-black">
      ...
      <div className="hidden md:flex flex-col items-center py-5 bg-[#F4F5F7] border-r border-[#E3E4E6] w-[60px] shrink-0 justify-between select-none">
        <div className="flex flex-col items-center space-y-5 w-full">
          ...
          <div className="relative group cursor-pointer hover:scale-105 transition-all">
            ...
            <div
              className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-yellow-400 text-xs flex items-center justify-center rounded-full text-black ring-1 ring-white"
              title="VIP Account"
            >
              ★
            </div>
        ...
        <div className="flex flex-col items-center space-y-4 w-full">
          ...
          <div className="relative group cursor-pointer hover:bg-gray-200 p-2 text-gray-500 rounded-xl">
            ...
            <div className="absolute left-[54px] top-1/2 -translate-y-1/2 scale-0 group-hover:scale-100 bg-white border-none text-gray-700 text-xs p-3 rounded-xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] z-50 whitespace-nowrap origin-left transition-all text-left">
              <span className="font-medium text-[#1a2b58] block mb-1">
                Alert Inbox
              </span>
              ...
              <div className="p-4 w-[240px]">
                ...
                <div className="mb-1">
                  <div className="flex items-center justify-between px-2 text-gray-400 text-sm uppercase tracking-widest mb-1 group">
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-gray-200 rounded text-gray-400 hover:text-gray-700 transition-colors hidden group-hover:block"
                      title="Add Project"
                    >
                    ...
                    >
                      <div className="relative flex-1">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setActiveEmojiPicker(
                              activeEmojiPicker === "folder" ? null : "folder",
                            )
                          }
                          className="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 rounded-full"
                        >
                      ...
                      <button
                        ...
                        className="text-white bg-green-500 p-1.5 rounded disabled:opacity-50"
                      >
                        <Check className="w-3 h-3" />
                      </button>
                      <button
                ...
                <div className="mt-8 border-t border-gray-200 pt-4">
                  ...
                  <div className="flex flex-wrap gap-1.5 px-2">
                    {allTags.map((tag) => {
                      const count = allActiveViewTodos.filter(t => !t.completed && t.tags && t.tags.includes(tag)).length;
                      return (
                        <button
                          key={tag}
                          onClick={() => {
      ...
      <main className="flex-1 overflow-y-auto bg-white flex flex-col items-center pb-24 md:pb-6 relative h-full">
        ...
        <div className="w-[98%] mx-auto px-6 py-5 md:py-6 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
            ...
            {activeAppTab === "tasks" && (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <button
                    onClick={() =>
                      handleUpdateDailyGoal(Math.max(1, dailyTaskGoal - 1))
                    }
                ...
                <div className="w-16 h-1.5 bg-gray-200/60 rounded-full overflow-hidden ml-1 hidden sm:block shrink-0">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (completedTodayCount / dailyTaskGoal) * 100)}%`,
                    }}
                  />
                </div>
                ...
                <span className="font-mono font-semibold text-gray-500 hidden md:inline shrink-0">
                  (
                  {Math.round(
                    Math.min(100, (completedTodayCount / dailyTaskGoal) * 100),
                  )}
                  %)
                </span>
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <button
              onClick={() => {
                setShowSmartTips(!showSmartTips);
            ...
            <div className="relative">
              ...
              {isHeaderMenuOpen && (
                <div className="absolute top-8 right-0 w-52 bg-white border-none rounded-xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] p-1.5 z-50 text-left scale-95 origin-top-right transition-transform animate-in fade-in duration-100">
                  ...
                  <button
                    onClick={() => {
                      setIsProgressBannerExpanded(!isProgressBannerExpanded);
                      setIsHeaderMenuOpen(false);
                    }}
                  ...
                  <button
                    onClick={async () => {
                      const completedInView = allActiveViewTodos.filter(
                        (t) => t.completed,
                      );
          ...
          <div className="w-[98%] mx-auto px-6 mt-1 flex justify-end">
            <div className="w-full max-w-[360px] bg-white border border-yellow-200 rounded-2xl shadow-2xl p-4 z-50 text-left animate-in fade-in slide-in-from-top-3 duration-200">
              ...
              <p className="font-medium text-base text-gray-500 leading-normal mb-3">
                Type any keyword in your task title to instantly auto-categorize
                into its perfect project!
              </p>
              <div className="space-y-2">
                <div className="text-xs bg-red-50/50 p-2 rounded-lg border border-red-100 flex items-start gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-gray-500 font-mono">
                      audit, ca, final, accounts, revision, test, exam
                    </span>
                  </div>
                </div>
                <div className="text-xs bg-purple-50/50 p-2 rounded-lg border border-purple-100 flex items-start gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-gray-500 font-mono">
                      spiritual, bible, meditation, pray, devotional
                    </span>
                  </div>
                </div>
                <div className="text-xs bg-blue-50/50 p-2 rounded-lg border border-blue-100 flex items-start gap-1.5">
                  ...
                  <div>
                    ...
                    <span className="text-gray-500 font-mono">
                      study, research, paper, assignment, read, lecture
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="w-[98%] mx-auto px-6 mt-1 flex justify-end absolute right-0 z-50">
            <div className="w-full max-w-[360px] bg-white border border-[#1a2b58]/10 rounded-2xl shadow-2xl p-4.5 text-left animate-in fade-in slide-in-from-top-3 duration-200">
              {(() => {
                ...
                const completedViewCount = allActiveViewTodos.filter(
                  (t) => t.completed,
                ).length;
                const completionPercent =
                  totalViewCount > 0
                    ? Math.round((completedViewCount / totalViewCount) * 100)
                    : 0;
                ...
                const circleDashoffset =
                  circleCircumference -
                  (completionPercent / 100) * circleCircumference;
              ...
              `}</style>
                    <div
                      style={
                        { "--banner-accent": projColor } as React.CSSProperties
                      }
                    >
                      <div className="flex items-center justify-between mb-3 border-b border-gray-50 pb-2">
                        ...
                        <button
                          ...
                          onClick={() => setIsProgressBannerExpanded(false)}
                        >
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-6">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-bold font-mono text-[#10B981] ml-1">
                                    {completedViewCount}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-bold font-mono text-blue-600 ml-1">
                                    {pendingViewCount}
                                  </span>
                                </div>
                              </div>
        ...
        <div className="w-[98%] mx-auto px-6 py-6 flex-1">
                ...
                <div className="w-full">
                  {currentViewType === "kanban" ? (
                    <div className="w-full">
                      ...
                      {(() => {
                        const projectTodos = allActiveViewTodos.filter(
                          (t) => !t.deletedAt,
                        );
                        const uniqueProjectTags = Array.from(
                          new Set(projectTodos.flatMap((t) => t.tags || [])),
                        );
                        ...
                        return (
                          <div className="bg-gray-50/50 border border-gray-150 rounded-2xl p-3.5 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-sm">
                            <div
                              className="flex items-center gap-2.5 overflow-x-auto select-none no-scrollbar"
                              ...
                              {uniqueProjectTags.map((tag) => {
                                ...
                                return (
                                  <button
                                    key={tag}
                                    onClick={() =>
                                      setKanbanSelectedTag(
                                        kanbanSelectedTag === tag ? null : tag,
                                      )
                                    }
                      ...
                      <div
                        className="w-full max-w-full overflow-x-auto text-left py-2"
                      ...
                      >
                        <div className="flex gap-6 items-start h-full pb-6 min-w-max select-none">
                          {(() => {
                            ...
                            const sectionsList = rawSections.includes(
                              "Not Sectioned",
                            )
                              ? rawSections
                            ...
                            return (
                              <>
                                {sectionsList.map((sectionName) => {
                                  const sectionTasks =
                                    allActiveViewTodos.filter((t) => {
                                      const matchesSection =
                                        sectionName === "Not Sectioned"
                                          ? !t.sectionName ||
                                            t.sectionName === "Not Sectioned"
                                          : t.sectionName === sectionName;
                                      if (!matchesSection) return false;
                                      if (kanbanSelectedTag) {
                                        return (
                                          t.tags &&
                                          t.tags.includes(kanbanSelectedTag)
                                        );
                                      }
                                      return true;
                                    });
                                  const inactiveTasks = sectionTasks.filter(
                                    (t) => !t.completed,
                                  );
                                  ...
                                  const isColumnCollapsed =
                                    collapsedKanbanColumns[sectionName] ??
                                    false;
                                  ...
                                  const isDraggingOver =
                                    draggingOverSection === sectionName;
                                  ...
                                  return (
                                    <div
                                      ...
                                      onDragEnter={() =>
                                        setDraggingOverSection(sectionName)
                                      }
                                      onDragLeave={() => {
                                      ...
                                      className={`w-[290px] min-h-[480px] bg-transparent rounded-2xl p-2.5 flex flex-col transition-all shrink-0 ${
                                        isDraggingOver
                                          ? "bg-blue-50/40 border-2 border-dashed border-blue-400/50"
                                    ...
                                    >
                                                        ...
                                                        <>
                                                          ...
                                                          <button
                                                            onClick={async () => {
                                                              ...
                                                              if (
                                                                confirm(
                                                                  `Are you sure you want to delete section "${sectionName}"? Tasks in this column will be moved to 'Not Sectioned'.`,
                                                                )
                                                              ) {
                                      ...
                                      <div
                                        className="space-y-1 flex-1 overflow-y-auto max-h-[460px] pr-1 pb-24"
                                        style={{ scrollbarWidth: "none" }}
                                      >
                                          ...
                                          <div className="bg-white border border-gray-150 rounded-2xl p-2.5 mb-3 shadow-md animate-in fade-in zoom-in-95 duration-100">
                                            <input
                                              ...
                                              onKeyDown={async (e) => {
                                                if (
                                                  e.key === "Enter" &&
                                                  newTaskTitleInline.trim()
                                                ) {
                                                  await handleAddTaskToSection(
                                                    sectionName,
                                                    newTaskTitleInline,
                                                    newTaskTagsInline,
                                                    newTaskDescInline,
                                                    newTaskSubtasksInline,
                                                  );
                                                  setNewTaskTitleInline("");
                                            ...
                                            {newTaskSubtasksInline.length >
                                              0 && (
                                              <div className="mt-1 space-y-1">
                                                {newTaskSubtasksInline.map(
                                                  (st, i) => (
                                                    <div
                                                      key={i}
                                                      className="flex items-center gap-1.5 text-xs text-gray-600 bg-gray-50/50 p-1 rounded-md px-2 border border-gray-100"
                                                    >
                                        ...
                                        {[
                                          {
                                            ...
                                            label: "P1 Urgent",
                                            borderLeft:
                                              "border-l-[3px] border-red-300 bg-red-50/40",
                                            text: "text-red-500",
                                          },
                                          {
                                            level: 2,
                                            label: "P2 High",
                                            borderLeft:
                                              "border-l-[3px] border-orange-300 bg-orange-50/40",
                                          ...
                                          },
                                          {
                                            ...
                                            label: "P3 Medium",
                                            borderLeft:
                                              "border-l-[3px] border-blue-300 bg-blue-50/40",
                                            text: "text-blue-500",
                                          },
                                          {
                                            level: 4,
                                            label: "P4 Standard",
                                            borderLeft:
                                              "border-l-[3px] border-slate-200 bg-slate-50/40",
                                            text: "text-slate-500",
                                          },
                                        ].map((prio) => {
                                          const prioTasks =
                                            inactiveTasks.filter(
                                              (t) =>
                                                (t.priority || 4) ===
                                                prio.level,
                                            );
                                          ...
                                          return (
                                            ...
                                            >
                                              <div
                                                className={`flex items-center justify-between px-2.5 py-1.5 w-full mb-2 rounded-md border ${prio.borderLeft} bg-white shadow-sm ${prio.text}`}
                                              >
                                                <div className="flex items-center gap-1.5 text-[10px] uppercase font-bold tracking-widest">
                                                  <span
                                                    className={`w-1.5 h-1.5 rounded-full ${prio.text.replace("text-", "bg-")}`}
                                                  />
                                              ...
                                              <div
                                                className={`space-y-2.5 flex-1 p-2 -mx-1 min-h-[40px] rounded-xl transition-all duration-200 ${draggingOverSection === sectionName + "-" + prio.level ? "bg-blue-50 border-2 border-dashed border-blue-300" : "bg-gray-50/30 border border-gray-100"}`}
                                              >
                                                {prioTasks.map((todo) => {
                                                  ...
                                                  if (
                                                    ...
                                                    borderPrio =
                                                      "border-red-300 hover:bg-red-50/50 text-red-400 bg-red-50/15";
                                                  } else if (
                                                    todo.priority === 2
                                                  ) {
                                                    borderPrio =
                                                      "border-orange-300 hover:bg-orange-50/50 text-orange-400 bg-orange-50/15";
                                                  } else if (
                                                    todo.priority === 3
                                                  ) {
                                                    borderPrio =
                                                      "border-blue-300 hover:bg-blue-50/50 text-blue-400 bg-blue-50/15";
                                                  }
                                                  ...
                                                  return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                          ...
                                                          {todo.subtasks &&
                                                            todo.subtasks
                                                              .length > 0 && (
                                                              <div
                                                                className="mt-1.5 flex items-center gap-1.5 text-[11px] text-gray-400 font-medium select-none"
                                                                onClick={(e) =>
                                                                  e.stopPropagation()
                                                                }
                                                              >
                                                      ...
                                                      {formattedDate && (
                                                        <div
                                                          className={`flex items-center gap-1.5 text-xs tracking-wide pl-[23px] mt-0.5 ${isOverdue ? "text-red-500 font-semibold" : "text-primary"}`}
                                                        >
                                                          <span>
                                                            {formattedDate}
                                                          </span>
                                                    ...
                                                    >
                                                      ...
                                                      <div className="min-w-0 flex-1">
                                                        <span className="text-xs font-medium text-gray-400 line-through truncate block flex items-center gap-1.5 mt-0.5">
                                                          {todo.repeatInterval && (
                                                            <RefreshCw className="inline-block w-3 h-3 text-gray-400 flex-shrink-0" />
                                                          )}
                                                          {getActiveDependenciesCount(
                                                            todo,
                                                          ) > 0 ? (
                    ...
                    <div className="w-full">
                            ...
                            <div className="relative flex-1 space-y-1.5">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  else if (/(weekly|every week)/.test(lower))
                                    setNewTaskRepeatInterval("weekly");
                                  else if (/(monthly|every month)/.test(lower))
                                    setNewTaskRepeatInterval("monthly");
                                  else if (
                                    /(quarterly|every quarter)/.test(lower)
                                  )
                                    setNewTaskRepeatInterval("quarterly");
                                  else if (
                                    /(yearly|every year|annually)/.test(lower)
                                  )
                                    setNewTaskRepeatInterval("yearly");
                                  else setNewTaskRepeatInterval(null);
                                }}
                                onKeyDown={(e) => {
                                  if (
                                    e.key === "Enter" &&
                            ...
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              <div className="relative">
                                ...
                                >
                                  ...
                                  <span>
                                    {newTaskDueDate
                                      ? format(newTaskDueDate, "MMM d, yyyy")
                                      : "No Date"}
                                  </span>
                                ...
                                <AnimatePresence>
                                  {showDatePicker && (
                                    <motion.div
                                      ...
                                      className="absolute left-0 mt-2 z-50 bg-white border-none rounded-2xl shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] p-1.5 w-44"
                                      onClick={(e) => e.stopPropagation()}
                                    >
                                      {[
                                        {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <span className="flex items-center gap-2">
                                              {" "}
                                              <span
                                                className={`w-1.5 h-1.5 rounded-full ${preset.color}`}
                                              />{" "}
                                              {preset.label}{" "}
                                            </span>{" "}
                                            {isSelected && (
                                      ...
                                      <button
                                        ...
                                        className={`w-full flex items-center justify-between px-2.5 py-2 text-xs font-medium rounded-lg transition-colors ${!newTaskDueDate ? "bg-primary/5 text-primary" : "hover:bg-gray-50 text-gray-700"}`}
                                      >
                                        {" "}
                                        <span className="flex items-center gap-2">
                                          {" "}
                                      ...
                                      <div className="px-1.5 pb-1">
                                        {" "}
                                        <label className="text-[10px] uppercase font-semibold text-gray-400 mb-1 block tracking-widest px-1">
                                          Custom Date
                                        </label>{" "}
                                        <input
                                          type="date"
                              ...
                              <div className="relative">
                                ...
                                >
                                  <Flag
                                    className={`w-3.5 h-3.5 ${getPriorityColor(newTaskPriority)} ${newTaskPriority < 4 ? "fill-current" : ""}`}
                                  />
                                      ...
                                      {[
                                        ...
                                        return (
                                          ...
                                          >
                                            <span className="flex items-center">
                                              <Flag
                                                className={`w-3.5 h-3.5 mr-2 ${getPriorityColor(level)} ${level < 4 ? "fill-current" : ""}`}
                                              />
                                              {label}
                                            </span>
                                            {isSelected && (
                              ...
                              <div className="relative">
                                ...
                                >
                                  <RefreshCw
                                    className={`w-3.5 h-3.5 shrink-0 ${newTaskRepeatInterval ? "text-primary" : "text-gray-400"}`}
                                  />
                                  <span>
                                    {newTaskRepeatInterval
                                      ? newTaskRepeatInterval
                                          .charAt(0)
                                          .toUpperCase() +
                                        newTaskRepeatInterval.slice(1)
                                      : "Repeat"}
                                  </span>
                                  <ChevronDown className="w-2.5 h-2.5 opacity-60" />
                                </button>
                              ...
                              <div className="relative">
                                ...
                                >
                                  <FileText
                                    className={`w-3.5 h-3.5 shrink-0 ${showNotesField || newTaskDesc ? "text-primary" : "text-gray-400"}`}
                                  />
                                  <span>Notes</span>
                                </button>
                              </div>
                              ...
                              <div className="relative">
                                <button
                                  type="button"
                                  onClick={() => {
                                    setShowSubtasksField(!showSubtasksField);
                                    setShowRepeatPicker(false);
                                    setShowPriorityPicker(false);
                                    setShowDatePicker(false);
                                  }}
                                ...
                                >
                                  <ListTodo
                                    className={`w-3.5 h-3.5 shrink-0 ${showSubtasksField || newTaskSubtasks.length > 0 ? "text-primary" : "text-gray-400"}`}
                                  />
                                  <span>Subtasks</span>
                                </button>
                              </div>
                              ...
                              {clients.length > 0 && (
                                <div className="relative">
                                  ...
                                  >
                                    <Users
                                      className={`w-3.5 h-3.5 shrink-0 ${newTaskClientId ? "text-primary" : "text-gray-400"}`}
                                    />
                                    <span>
                                      {newTaskClientId
                                        ? clients.find(
                                            (c) => c.uid === newTaskClientId,
                                  ...
                                  <AnimatePresence>
                                    {showClientPicker && (
                                      ...
                                      >
                                        ...
                                        {clients.map((c) => {
                                          const isSelected =
                                            newTaskClientId === c.uid;
                                          return (
                                            <button
                                              key={c.uid}
                                              type="button"
                                              onClick={() => {
                                                setNewTaskClientId(c.uid);
                                                setShowClientPicker(false);
                                              }}
                                              className={`w-full flex items-center justify-between px-2.5 py-2 text-xs font-medium rounded-lg transition-colors ${isSelected ? "bg-primary/5 text-primary" : "hover:bg-gray-50 text-gray-700"}`}
                                            >
                                ...
                                {newTaskSubtasks.length > 0 && (
                                  <div className="space-y-1.5 max-h-36 overflow-y-auto">
                                    {newTaskSubtasks.map((st, i) => (
                                      <div
                                        key={i}
                                        className="flex items-center gap-2 text-xs text-gray-700 bg-white p-1.5 rounded-lg px-2.5 border border-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.01)]"
                                      >
                                  ...
                                  <div className="space-y-4 relative z-10">
                                    <div className="grid grid-cols-2 gap-2">
                                      <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-2.5 text-center">
                                        <span className="block text-[10px] text-emerald-600 font-bold uppercase tracking-wider mb-1">
                                          Today
                                        </span>
                                    ...
                                    <div className="flex items-center gap-3 bg-orange-50/50 border border-orange-100 p-2.5 rounded-xl">
                                      ...
                                      <div>
                                        ...
                                        <span className="block text-[10px] font-medium text-orange-700/80">
                              ...
                              if (upcomingDeadlines.length === 0) {
                                return (
                                  <p className="text-xs text-gray-500 italic relative z-10">
                                    No pressing deadlines in the next 48 hours.
                                  </p>
                                );
                              }
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative z-10">
                                  {upcomingDeadlines.slice(0, 6).map((task) => {
                                    ...
                                    return (
                                      <div
                                        key={task.id}
                                        className="bg-white/80 backdrop-blur border border-white/50 p-3 rounded-xl shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer flex flex-col justify-between min-h-[72px]"
                                        onClick={() =>
                                      ...
                                      >
                                        <div className="flex items-start justify-between gap-2 mb-1.5">
                                          <span
                                            className="text-xs font-normal text-gray-900 truncate leading-tight"
                                            title={task.title}
                                          >
                                            {task.title}
                                          </span>
                                          <span
                                            className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider shrink-0 ${isOverdue ? "bg-red-100 text-red-600" : hoursLeft < 24 ? "bg-orange-100 text-orange-600" : "bg-blue-100 text-blue-600"}`}
                                          >
                                            {isOverdue
                                              ? "Overdue"
                                              : hoursLeft === 0
                                  ...
                                  {upcomingDeadlines.length > 6 && (
                                    <div
                                      className="flex items-center justify-center p-3 rounded-xl border border-dashed border-[#1a2b58]/20 bg-white/30 text-xs font-semibold text-[#1a2b58]/70 hover:text-[#1a2b58] cursor-pointer hover:bg-white/60 transition-colors h-full min-h-[72px]"
                                      onClick={() => setViewMode("upcoming")}
                                    >
                                      +{upcomingDeadlines.length - 6} more
                                      deadlines
                                    </div>
                                  )}
                                </div>
                          ...
                          <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm relative overflow-hidden flex flex-col justify-between">
                            ...
                            <div>
                              ...
                              {(() => {
                                ...
                                const p1Count = todaysTasks.filter(
                                  (t) => t.priority === 1,
                                ).length;
                                const p2Count = todaysTasks.filter(
                                  (t) => t.priority === 2,
                                ).length;
                                const p3Count = todaysTasks.filter(
                                  (t) => t.priority === 3,
                                ).length;
                                const p4Count = todaysTasks.filter(
                                  (t) => !t.priority || t.priority === 4,
                                ).length;
                                ...
                                return (
                                  <div className="space-y-3 relative z-10">
                                    <div className="flex items-center gap-2">
                                      <div className="w-6 text-[10px] font-bold text-gray-500">
                                        P1
                                      </div>
                                      <div className="flex-1 h-2.5 bg-gray-100 rounded-full overflow-hidden">
                                        <div
                                          ...
                                          style={{
                                            width: `${(p1Count / totalCount) * 100}%`,
                                          }}
                                        />
                                      </div>
                                      <div className="w-4 text-[10px] font-semibold text-gray-700 text-right">
                                        {p1Count}
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                      ...
                      {allActiveViewTodos.filter(
                        ...
                        viewMode !== "completed" && (
                          <div className="mb-6">
                            <button
                              onClick={() =>
                                setIsCountdownExpanded(!isCountdownExpanded)
                              }
                      ...
                      {viewMode === "today" ? (
                        (() => {
                          const activeTodos = allActiveViewTodos.filter(
                            (t) => !t.completed,
                          );
                          const overdueList = activeTodos.filter((t) => {
                            ...
                            return (
                              dateVal &&
                              dateVal < startOfDay(new Date()).getTime()
                            );
                          });
                                  ...
                                  ? (() => {
                                      ...
                                      return folderProjects.map((project) => {
                                        ...
                                        return (
                                          <div
                                            key={project.id}
                                            className="mb-6 last:mb-2 text-left"
                                          >
                                            <div
                                            ...
                                            <div
                                              className="pl-3.5 space-y-0.5 border-l-[3px] ml-3 mb-4 text-left"
                                              style={{
            ...
            <div className="mt-4 border border-[#e5e7eb] bg-white rounded-xl shadow-lg p-4 text-left">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (/(daily|every day)/.test(lower))
                      ...
                      setNewTaskRepeatInterval("monthly");
                    else if (/(quarterly|every quarter)/.test(lower))
                      setNewTaskRepeatInterval("quarterly");
                    else if (/(yearly|every year|annually)/.test(lower))
                      setNewTaskRepeatInterval("yearly");
                    else setNewTaskRepeatInterval(null);
                  }}
                  onKeyDown={(e) => {
              ...
              <div className="flex justify-end space-x-2">
                <button
                  ...
                  className="px-3 py-1.5 text-xs hover:bg-gray-100 text-gray-600 rounded"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={!newTaskTitle.trim()}
                  className="px-3 py-1.5 text-xs bg-primary text-white rounded disabled:opacity-50 font-medium"
                >
                  Add Task
                </button>
              </div>
          ...
          {activeAppTab === "matrix" && (
            <EisenhowerMatrix
              ...
              onAutoCategorize={(matchedName) => {
                setAutoProjectNotice(
                  `Auto-categorized task to "${matchedName}"`,
                );
                setTimeout(() => setAutoProjectNotice(null), 4000);
              }}
            ...
            <div className="text-left">
              <div className="mb-4">
                ...
                <p className="font-medium text-base text-gray-500">
                  Focus solely on critical urgent P1 tasks.
                </p>
              </div>
              <div className="space-y-2">
                {todos
                  .filter(
                    (t) => !t.completed && !t.deletedAt && t.priority === 1,
                  )
                  .map((todo) => (
                    <div
                      key={todo.id}
                      onClick={() => setSelectedTodoId(todo.id)}
                      className="p-3.5 bg-red-50/30 border border-red-100 rounded-xl hover:shadow transition-all hover:-translate-y-0.5 cursor-pointer flex justify-between items-center"
                    >
                      <div className="flex items-center space-x-2.5">
                      ...
                      <span className="text-xs bg-red-100/75 text-red-700 font-medium px-2 py-0.5 rounded-full uppercase">
                        P1 High Target
                      </span>
                    </div>
                  ))}
                {todos.filter(
                  ...
                  <div className="text-center py-12 border rounded-xl border-dashed">
                    ...
                    <span className="text-xs text-gray-400">
                      No critical high-priorities remaining! Excellent.
                    </span>
                  </div>
                )}
              </div>
          ...
          {activeAppTab === "dependencies" && <DependencyTree todos={todos} />}
            ...
            <div className="text-left w-full h-full max-w-3xl mx-auto py-2 focus:outline-none">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <p className="font-normal text-sm text-gray-400 mt-1">
                    Manage your offline backup policies, view synchronization
                    metrics, and request a database dump.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl">
                  ...
                  <span className="text-xs text-indigo-400 font-medium block mt-1">
                    ★ Multi-view hubs loaded
                  </span>
                </div>
                <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl">
                  <span className="text-xs text-gray-400 uppercase tracking-widest block font-medium">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm mb-6">
                <div className="p-5 border-b border-slate-800 bg-slate-950">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Define your daily task threshold. Progress will be displayed
                    in real-time in the main dashboard header to keep you
                    motivated.
                  </p>
                </div>
                ...
                <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition-colors">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5 max-w-xl">
                      We recommend setting a realistic target (e.g., 3 to 7
                      tasks completed per day). Any change will sync
                      automatically to your cloud profile.
                    </p>
                  </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm mb-6">
                <div className="p-5 border-b border-slate-800 bg-slate-950">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Export your complete workspace as standard UTF-8 encoded
                    comma-separated structures. Ideal for offline storage,
                    backups, or raw integration.
                  </p>
                </div>
                ...
                <div className="divide-y divide-slate-800 text-sm font-sans">
                  ...
                  <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition-colors">
                    <div>
                      <div className="font-semibold text-white flex items-center gap-1.5">
                        <span className="p-1 bg-red-950 text-red-400 rounded font-mono text-center leading-none">
                          ★
                        </span>
                        Universal Multi-Dossier Consolidated Backup
                      </div>
                      <p className="text-xs text-gray-400 mt-0.5 max-w-xl">
                        A comprehensive relational dump merging folders trees,
                        custom projects registries, and tasks databases in a
                  ...
                  <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition-colors">
                    <div>
                      <div className="font-semibold text-white flex items-center gap-1.5">
                        <span className="p-1 bg-green-950 text-green-400 rounded font-mono text-center leading-none">
                          ✓
                        </span>
                        Checklist Tasks Sheet
                      </div>
                      <p className="text-xs text-gray-400 mt-0.5 max-w-xl">
                        Downloads lists of tasks with fields: Description notes,
                        Completed Status, Priorities, assigned Project Names,
                        Tag collections, and timestamp records.
                      </p>
                    </div>
                    <button
                  ...
                  <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition-colors">
                    <div>
                      <div className="font-semibold text-white flex items-center gap-1.5">
                        <span className="p-1 bg-blue-950 text-blue-400 rounded font-mono text-center leading-none">
                          ⊞
                        </span>
                        Workspace Projects Sheet
                      </div>
                      <p className="text-xs text-gray-400 mt-0.5 max-w-xl">
                        Downloads lists of all your customized projects,
                        assigned colors, display icons, custom views
                        (Kanban/List/Timeline), and corresponding folders.
                      </p>
                    </div>
                    <button
                  ...
                  <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-850/50 transition-colors">
                    <div>
                      <div className="font-semibold text-white flex items-center gap-1.5">
                        <span className="p-1 bg-purple-950 text-purple-400 rounded font-mono text-center leading-none">
                          ⚙
                        </span>
                        Structural Folders Registry
                      </div>
                      <p className="text-xs text-gray-400 mt-0.5 max-w-xl">
                        Downloads lists of high-level folder organization trees
                        and associated directory colors. Perfect for duplicating
                        layout schemes.
                      </p>
                    </div>
                    <button
              ...
              <div className="p-4 bg-blue-950/40 border border-blue-900/50 rounded-2xl flex items-start gap-3 mt-4">
                ...
                <p className="text-xs text-blue-300 leading-relaxed">
                  <strong>Security Note:</strong> All export procedures are
                  fully client-side and happen locally inside your browser
                  context. No personal tasks, projects description, or folder
                  hierarchies are shared with third-party networks during backup
                  dumps.
                </p>
              </div>
            </div>
          ...
          {activeAppTab === "search" && (
            <div className="text-left w-full">
              <div className="mb-6">
                ...
                <p className="font-medium text-base text-gray-500">
                  Perform deep filters across dates, tags, and projects.
                </p>
              </div>
                ...
                {todos
                  ...
                  .map((todo) => (
                    <div
                      key={todo.id}
                      onClick={() => setSelectedTodoId(todo.id)}
                      className="p-3 bg-white border border-gray-100 hover:border-[#1a2b58] hover:-translate-y-0.5 transition-all cursor-pointer rounded-xl flex justify-between items-center"
                    >
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 font-medium rounded-full ${todo.completed ? "bg-gray-100 text-gray-400" : "bg-green-100 text-green-700"}`}
                      >
                        {todo.completed ? "Completed" : "Active"}
                      </span>
                    </div>
            ...
            >
              {(() => {
                ...
                const logDueDateChange = (nextDate: number | null) => {
                  const now = Date.now();
                  const oldVal = todo.dueDate
                    ? format(new Date(todo.dueDate), "MMM d, yyyy")
                    : "No Date";
                  const newVal = nextDate
                ...
                const getActivityDetails = (act: TaskActivity) => {
                  switch (act.type) {
                    case "status":
                      return {
                        label:
                          act.newValue === "Completed"
                            ? "completed the task"
                            : "marked the task as active",
                    ...
                    case "priority":
                      return {
                        ...
                        icon: (
                          <Flag className="w-3 h-3 text-blue-600 fill-current" />
                        ),
                    ...
                    <div className="flex items-center justify-between border-b border-gray-100 p-4">
                      <div className="flex items-center text-xs font-medium text-gray-500">
                        {todo.projectId && todo.projectId !== "inbox" ? (
                          <>
                            <Folder className="w-3.5 h-3.5 mr-1.5 text-[#1a2b58]" />
                            {
                          ...
                          <>
                            <Inbox className="w-3.5 h-3.5 mr-1.5 text-gray-400" />
                            Inbox
                          </>
                        )}
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto p-6 text-left">
                              ...
                              {activeEmojiPicker === `todo-${todo.id}` && (
                                ...
                                >
                                  <EmojiPicker
                                    onEmojiClick={(emojiData) => {
                                      todoService.updateTodo(todo.id, {
                                        title: todo.title + emojiData.emoji,
                                      });
                                      setActiveEmojiPicker(null);
                                    }}
                                    width={280}
                        ...
                        <div className="grid grid-cols-4 gap-2.5 mt-5 border-b border-gray-150 pb-5">
                          ...
                          <div className="relative">
                            ...
                            >
                              <span
                                className={`truncate flex items-center gap-1.5 ${todo.dueDate && todo.dueDate < startOfDay(new Date()).getTime() ? "text-red-500 font-semibold" : ""}`}
                              >
                                <CalendarIcon
                                  className={`w-3.5 h-3.5 ${todo.dueDate && todo.dueDate < startOfDay(new Date()).getTime() ? "text-red-500" : todo.dueDate ? "text-primary" : "text-gray-400"}`}
                                />
                                {todo.dueDate
                                  ? format(
                              ...
                              <ChevronDown
                                className={`w-3.5 h-3.5 text-gray-400 shrink-0 ml-1 transition-transform duration-200 ${showDetailDatePicker ? "rotate-180" : ""}`}
                              />
                            </button>
                            <AnimatePresence>
                          ...
                          <div className="relative">
                            ...
                            >
                              <span className="flex items-center gap-1.5 truncate">
                                <Flag
                                  className={`w-3.5 h-3.5 ${getPriorityColor(todo.priority || 4)} fill-current`}
                                />
                                P{todo.priority || 4}
                              </span>
                              <ChevronDown
                                className={`w-3.5 h-3.5 text-gray-400 shrink-0 ml-1 transition-transform duration-200 ${showDetailPriorityPicker ? "rotate-180" : ""}`}
                              />
                            </button>
                            <AnimatePresence>
                              {showDetailPriorityPicker && (
                          ...
                          <div className="relative">
                            ...
                            >
                              <span className="flex items-center gap-1.5 truncate">
                                <RefreshCw
                                  className={`w-3.5 h-3.5 ${todo.repeatInterval ? "text-primary" : "text-gray-400"}`}
                                />
                                {todo.repeatInterval
                                  ? todo.repeatInterval
                                      .charAt(0)
                                      .toUpperCase() +
                                    todo.repeatInterval.slice(1)
                                  : "None"}
                              </span>
                              <ChevronDown
                                className={`w-3.5 h-3.5 text-gray-400 shrink-0 ml-1 transition-transform duration-200 ${showDetailRepeatPicker ? "rotate-180" : ""}`}
                              />
                            </button>
                            <AnimatePresence>
                              {showDetailRepeatPicker && (
                                  ...
                                  {[
                                    ...
                                    return (
                                      ...
                                      >
                                        <span className="flex items-center">
                                          <RefreshCw
                                            className={`w-3.5 h-3.5 mr-2 ${isSelected ? "text-primary" : "text-gray-400"}`}
                                          />
                                          {label}
                                        </span>
                                        {isSelected && (
                          ...
                          <div className="relative">
                            ...
                            >
                              <span className="flex items-center gap-1.5 truncate">
                                <Users
                                  className={`w-3.5 h-3.5 ${todo.clientId ? "text-primary" : "text-gray-400"}`}
                                />
                                {todo.clientId
                                  ? clients.find((c) => c.uid === todo.clientId)
                                      ?.displayName ||
                                    todo.clientName ||
                                    "Client"
                                  : "None"}
                              ...
                              <ChevronDown
                                className={`w-3.5 h-3.5 text-gray-400 shrink-0 ml-1 transition-transform duration-200 ${showDetailClientPicker ? "rotate-180" : ""}`}
                              />
                            </button>
                            <AnimatePresence>
                              {showDetailClientPicker && (
                        ...
                        <div className="relative mt-4">
                          ...
                          >
                            <span className="flex items-center gap-1.5 truncate">
                              <Lock
                                className={`w-3.5 h-3.5 ${(todo.blockedBy?.length || 0) > 0 ? "text-rose-500" : "text-gray-400"}`}
                              />
                              {(todo.blockedBy?.length || 0) > 0
                                ? `${todo.blockedBy?.length} Task(s)`
                                : "None"}
                            </span>
                            <ChevronDown
                              className={`w-3.5 h-3.5 text-gray-400 shrink-0 ml-1 transition-transform duration-200 ${showDetailBlockingPicker ? "rotate-180" : ""}`}
                            />
                          </button>
                          <AnimatePresence>
                            {showDetailBlockingPicker && (
                                    ...
                                    .map((t) => {
                                      ...
                                      return (
                                        <button
                                          key={t.id}
                                          onClick={() => {
                                            const currentBlockedBy =
                                              todo.blockedBy || [];
                                            let nextBlockedBy;
                                            if (isSelected) {
                                              nextBlockedBy =
                                                currentBlockedBy.filter(
                                                  (id) => id !== t.id,
                                                );
                                            } else {
                                              nextBlockedBy = [
                                        ...
                                        >
                                          <span className="flex items-center truncate max-w-[180px]">
                                            <Lock
                                              className={`w-3.5 h-3.5 mr-2 shrink-0 ${isSelected ? "text-rose-500" : "text-gray-400"}`}
                                            />
                                            <span className="truncate">
                                              {t.title}
                                            </span>
                                          ...
                                          {isSelected && (
                                            <Check className="w-3.5 h-3.5 text-rose-500 shrink-0 ml-2" />
                                          )}
                                        </button>
                                      );
                                    })
                        ...
                        <div className="mt-5 space-y-5">
                          ...
                          <div className="flex flex-col text-left">
                            <label className="text-xs text-gray-400 uppercase tracking-widest mb-2">
                              Subtasks
                            </label>
                              ...
                              >
                                {todo.subtasks?.map((subtask) => (
                                  ...
                                  >
                                    ...
                                    <button
                                      onClick={() => {
                                        const next = todo.subtasks?.filter(
                                          (s) => s.id !== subtask.id,
                                        );
                                        todoService.updateTodo(todo.id, {
                                          subtasks: next,
                          ...
                          <div className="flex flex-col text-left pt-5 border-t border-gray-150">
                            <label className="text-xs text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5 select-none font-medium">
                              <MessageSquare className="w-3.5 h-3.5 text-[#1a2b58]" />{" "}
                              Comments (
                              {(todo.comments?.length || 0) +
                                (todo.comments?.reduce(
                              ...
                              {!todo.comments || todo.comments.length === 0 ? (
                                ...
                                todo.comments.map((comment) => {
                                  ...
                                  return (
                                    <div
                                      key={comment.id}
                                      className="group/comment flex flex-col gap-1.5"
                                    >
                                      {/* Comment Bubble */}
                                      <div className="bg-gray-50/70 p-3 rounded-xl border border-gray-100 flex flex-col gap-1 text-xs transition-colors hover:bg-gray-50">
                                        ...
                                        <p className="text-gray-700 whitespace-pre-wrap select-text">
                                          {comment.text}
                                        </p>
                                        ...
                                        <div className="flex items-center gap-4 mt-1 pt-1.5 border-t border-gray-100/50 text-[11px] text-gray-400">
                                          ...
                                          >
                                            <Heart
                                              className={`w-3 h-3 ${hasLikedComment ? "fill-rose-500 text-rose-500" : ""}`}
                                            />
                                          ...
                                          <div className="border-l-2 border-gray-100 pl-3.5 ml-3.5 space-y-2 flex flex-col">
                                            {comment.replies.map((reply) => {
                                              ...
                                              return (
                                                <div
                                                  key={reply.id}
                                                  className="bg-gray-50/40 p-2.5 rounded-xl border border-gray-50 flex flex-col gap-1 text-[11px] transition-colors hover:bg-gray-50/60"
                                                >
                                                  <div className="flex items-center justify-between text-gray-400 font-medium select-none">
                                                    <span className="text-gray-600 font-semibold">
                                                      {reply.author || "You"}
                                                  ...
                                                  <div className="flex items-center gap-3 mt-0.5 text-[10px] text-gray-400">
                                                    ...
                                                    >
                                                      <Heart
                                                        className={`w-2.5 h-2.5 ${hasLikedReply ? "fill-rose-500 text-rose-500" : ""}`}
                                                      />
                                                      <span>
                                                        {reply.likes || 0}
                                                      </span>
                        ...
                        {!todo.activities || todo.activities.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center text-center py-12 px-4">
                            ...
                            <p className="text-xs text-gray-400 max-w-[260px] leading-relaxed">
                              Changes to this task's status, priority, or title
                              ...
                              .map((act) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div
                                      className={`absolute -left-[25px] top-0.5 w-5 h-5 rounded-full border flex items-center justify-center shrink-0 bg-white ${details.bgColor}`}
                                    >
                                      {details.icon}
                                    </div>
      ...
      {showHelpGuide && <GuidePopup onClose={() => setShowHelpGuide(false)} />}
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg text-gray-800">
                      {editingProjectIdInModal ? "Edit List" : "Add List"}
                    </h3>
                      ...
                      {!isCreatingFolderInModal ? (
                        ...
                        <div className="relative flex-1 max-w-[280px] flex items-center gap-1.5 animate-in fade-in slide-in-from-left-1 duration-150">
                          <input
                            ...
                            onChange={(e) =>
                              setNewFolderNameInModal(e.target.value)
                            }
                            onKeyDown={async (e) => {
                              if (e.key === "Enter") {
                                e.preventDefault();
                                if (
                                  newFolderNameInModal.trim() &&
                                  auth.currentUser
                                ) {
                                  const result = await todoService.createFolder(
                                    newFolderNameInModal.trim(),
                                    auth.currentUser.uid,
                                  );
                                  if (result && result.id) {
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="relative flex-1 max-w-[280px] group z-[40]">
                        <CustomSelect
                          ...
                          onChange={(val) =>
                            setListSmartOption(val as "all" | "none")
                          }
                          className="flex-1 w-full text-xs bg-white border border-gray-200 hover:border-blue-400 hover:shadow-md rounded-xl py-2 px-3 font-medium text-gray-700 shadow-sm transition-colors cursor-pointer relative z-0"
                          options={[
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (newProjectName.trim() && auth.currentUser) {
                        const targetFolder =
                          listFolderId === "none" ? null : listFolderId;
                        const sections = [];
                        if (editingProjectIdInModal) {
                          await todoService.updateProject(
                            editingProjectIdInModal,
                            {
        ...
        {confirmDialog?.isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/25 backdrop-blur-sm">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="bg-white rounded-2xl p-6 shadow-2xl max-w-sm w-full border text-left"
            >
              <h3 className=" text-sm text-gray-900 mb-2">Confirm Action</h3>
              <p className="font-medium text-base text-gray-500 mb-5">
                {confirmDialog.message}
              </p>
              <div className="flex justify-end space-x-2">
                <button
                  onClick={() => setConfirmDialog(null)}
                  className="px-3 py-1.5 text-xs bg-gray-100 hover:bg-gray-200 rounded font-medium"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDialog.onConfirm}
                  className="px-3 py-1.5 text-xs bg-red-600 text-white rounded font-medium hover:bg-red-700"
                >
                  Confirm
                </button>
              </div>
            ...
            >
              ...
              <div className="space-y-4">
                <p className="text-xs text-gray-500 font-medium leading-relaxed">
                  Please enter or select a reason for declining or deleting this
                  task. This reason will be persistently saved in the task's
                  audit metadata.
                </p>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setTaskDeclineOrDeleteReason(e.target.value)
                    }
