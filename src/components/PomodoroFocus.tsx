  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-white border border-[#f0f0f0] rounded-2xl shadow-xl p-8 text-center mt-6">
      <div className="mb-6">
        ...
        <h2 className="text-2xl font-black text-gray-900 mt-2">Time to Focus</h2>
      ...
      <div className="flex justify-center space-x-2 bg-gray-50 p-1 rounded-xl mb-8 max-w-md mx-auto">
        <button
          ...
          className={`px-4 py-2 text-xs font-bold uppercase rounded-lg transition-all ${timerMode === 'work' ? 'bg-white shadow-sm text-[#2F6BE6]' : 'text-gray-500 hover:text-gray-700'}`}
        >
          Work Loop (25m)
        </button>
        <button
          onClick={() => { setTimerMode('break'); setTimeRemaining(5 * 60); setTimerRunning(false); }}
          className={`px-4 py-2 text-xs font-bold uppercase rounded-lg transition-all ${timerMode === 'break' ? 'bg-white shadow-sm text-[#2F6BE6]' : 'text-gray-500 hover:text-gray-700'}`}
        >
          Break Progress (5m)
        </button>
        <button
          onClick={() => { setTimerMode('longBreak'); setTimeRemaining(15 * 60); setTimerRunning(false); }}
          className={`px-4 py-2 text-xs font-bold uppercase rounded-lg transition-all ${timerMode === 'longBreak' ? 'bg-white shadow-sm text-[#2F6BE6]' : 'text-gray-500 hover:text-gray-700'}`}
        >
          Long Break (15m)
        </button>
      </div>
      ...
      <div className="relative w-64 h-64 mx-auto mb-8 flex items-center justify-center">
        <svg className="absolute w-full h-full -rotate-90">
          ...
          <circle
            ...
            cy="128"
            r="110"
            className="stroke-[#2F6BE6] fill-none transition-all duration-300"
            strokeWidth="8"
      ...
      <div className="flex justify-center items-center space-x-4 mb-8">
        ...
        <button
          ...
          className={`px-8 py-3.5 rounded-full text-white font-bold text-sm shadow-md transition-all ${timerRunning ? 'bg-gray-800 hover:bg-gray-900' : 'bg-[#2F6BE6] hover:bg-[#1a56cc]'}`}
        >
      ...
      <div className="border-t border-gray-100 pt-6">
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Synthesizer Atmosphere</h4>
        <div className="flex justify-center space-x-3">
          <button
            onClick={() => handleSoundToggle('rain')}
            className={`flex items-center space-x-2 px-4 py-2 text-xs font-semibold border rounded-full transition-all ${ambientSound === 'rain' ? 'bg-[#2F6BE6] text-white border-[#2F6BE6]' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200'}`}
          >
          ...
          <button
            ...
            className={`flex items-center space-x-2 px-4 py-2 text-xs font-semibold border rounded-full transition-all ${ambientSound === 'campfire' ? 'bg-[#eab308]/10 text-[#a16207] border-[#eab308]/30 font-bold' : 'bg-white hover:bg-gray-50 text-gray-700 border-gray-200'}`}
          >
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-3.5 mt-6 flex items-center justify-between">
        <div className="text-left">
          <span className="text-[10px] font-bold text-gray-400 uppercase block tracking-wider">Today's Balance</span>
          <span className="text-sm font-bold text-gray-700">{focusMinutesLogged} minutes logged</span>
        </div>
