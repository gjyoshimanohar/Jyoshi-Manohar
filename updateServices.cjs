...
c = c.replace(
  ...
  'className={`group relative overflow-hidden block p-8 lg:p-10 rounded-3xl h-full ${bgClass} border border-white/50 backdrop-blur-sm transition-all duration-500 hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] hover:-translate-y-2 hover:scale-[1.02]`}'
);
...
const tIconsRep = `<div className="absolute inset-0 bg-gradient-to-br from-white/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
                  {IconComponent && (
                    <IconComponent 
                      className="relative z-10 h-12 w-12 text-primary mb-8 bg-white p-3 rounded-2xl shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3 group-hover:bg-primary group-hover:text-white" 
                      strokeWidth={1.2} 
                    />
                  )}
