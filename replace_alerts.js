  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      if (content.includes('alert(')) {
        ...
        content = content.replace(/toast\("([Ff]ailed|[Ee]rror|[Pp]lease|[Ii]nvalid|[Nn]o )/g, 'toast.error("$1');
        content = content.replace(/toast\(\`/g, 'toast.success(`'); // Default template strings to success unless error text
        content = content.replace(/toast\.success\(\`([Ff]ailed|[Ee]rror|[Pp]lease|[Ii]nvalid|[Nn]o )/g, 'toast.error(`$1');
